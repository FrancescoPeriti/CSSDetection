...
# I am not the original author of this file. All credits go to the original contributors.
#
# If you have any questions or issues, please refer to the original repository.
...
def cluster_correlation_search(G, s = 10, max_attempts = 200, weight_threshold=0.0, max_iters = 5000, initial = [], split_flag = True):
    """
    ...
    if initial == []: # initialize with connected components unless initial clustering is provided
        classes = cluster_connected_components(G,weight_threshold)
    else:
        classes = initial
    ...
    edges_positive = set([(n2i[i],n2i[j],G[i][j]['weight']) for (i,j) in G.edges() if G[i][j]['weight'] >= weight_threshold])
    edges_negative = set([(n2i[i],n2i[j],G[i][j]['weight']) for (i,j) in G.edges() if G[i][j]['weight'] < weight_threshold])
    ...
    if split_flag: classes = split_non_evidence_clusters(G, weight_threshold, classes)
...
def cluster_connected_components(G, weight_threshold, is_non_value=lambda x: np.isnan(x)):
    """
    ...
    edges_negative = [(i,j) for (i,j) in G.edges() if G[i][j]['weight'] < weight_threshold or is_non_value(G[i][j]['weight'])]
...
def split_non_evidence_clusters(G, weight_threshold, clusters, is_non_value=lambda x: np.isnan(x)):
    """
    ...
    for cluster in clusters:
        ...
        components = cluster_connected_components(subgraph,weight_threshold)
